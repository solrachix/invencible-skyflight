...
assert.match(html, /const INTRO_CAMERA_START_DISTANCE = 150;/);
assert.match(html, /const INTRO_CAMERA_END_DISTANCE = 430;/);
...
assert.match(html, /const CAMERA_FOLLOW_DISTANCE = 360;/);
assert.match(html, /const CAMERA_FOLLOW_HEIGHT = 130;/);
assert.match(html, /const CAMERA_LOOK_AHEAD = 340;/);
...
assert.match(html, /const normalFollowDistance = THREE\.MathUtils\.lerp\([\s\S]*?CAMERA_FOLLOW_DISTANCE,[\s\S]*?900,[\s\S]*?speedRatio,[\s\S]*?\);/);
assert.match(html, /const normalFollowHeight = THREE\.MathUtils\.lerp\([\s\S]*?CAMERA_FOLLOW_HEIGHT,[\s\S]*?300,[\s\S]*?speedRatio,[\s\S]*?\);/);
