...
assert.match(html, /function updateCompass\(\)/);
assert.match(html, /<title>Invencível voando sobre o mundo 🌍<\/title>/);
...
assert.match(html, /renderer\.domElement\.addEventListener\("click", handleCanvasClick\);/);
...
assert.match(html, /introState\.mouseCaptured = document\.pointerLockElement === element;/);
assert.match(html, /if \(document\.pointerLockElement === element\) return;/);
...
assert.match(html, /Troque com tk \/ sp \/ ny/);
...
assert.match(html, /<div>WASD fly<\/div>/);
assert.match(html, /<div>double W boost<\/div>/);
assert.match(html, /<div>Space up<\/div>/);
assert.match(html, /<div>double Space climb<\/div>/);
assert.match(html, /<div>Shift down<\/div>/);
assert.match(html, /<div>C camera<\/div>/);
assert.doesNotMatch(html, /hero-status/);
